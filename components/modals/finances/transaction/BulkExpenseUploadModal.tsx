...
export function BulkExpenseUploadModal({ closeModal, accounts, categories }: any) {
  ...
  const handleDownload = async () => {
    ...
    await generateExpenseBulkTemplatePDF(
      expAccounts.map((a: any) => ({ _id: String(a._id), name: a.name, code: a.code })),
  ...
  return (
    <div className="w-full max-w-full sm:max-w-[95vw] md:max-w-[860px] lg:max-w-[960px] max-h-[78dvh] flex flex-col gap-4 overflow-x-hidden">
      ...
      <div className="bg-shaded/60 border border-border rounded-xl p-3 text-[10px] leading-relaxed text-text-muted">
        <span className="font-black text-danger">Compulsory *: Head (name, case-insensitive, must already exist), Amount, Date BS or AD (YYYY-MM-DD, e.g. 2082-05-17 BS auto-converts).</span> Description optional. Type locked EXPENSE. BS dates auto-converted to AD. Heads must be created first in Finance → Chart of Accounts (strict, case-insensitive). Empty rows ignored. Multifile .xlsx/.pdf — 15 MB per file, 30 MB total.
      </div>
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap gap-2 text-[11px] font-black uppercase tracking-widest">
            ...
            {(((parseState as any).unknownCount ?? (parseState as any).willCreateCount) > 0) && (
              <span className="px-3 py-1 rounded-full bg-danger/10 text-danger border border-danger/20">Unknown heads: {((parseState as any).unknownCount ?? (parseState as any).willCreateCount)}</span>
            )}
          </div>
          ...
          {invalidRows.length > 0 && <p className="text-[10px] text-danger font-bold">{invalidRows.length} invalid rows have unknown head (strict, case-insensitive — create them first in Chart of Accounts) or other errors. Fix the file and re-upload.</p>}
          ...
          {(commitState as any)?.success && <div className="text-[11px] font-bold text-success bg-success/10 border border-success/20 rounded-lg p-3">✓ Imported {(commitState as any).inserted} expenses (strict heads, case-insensitive). Refreshing...</div>}
        </div>
      )}
    </div>
  );
