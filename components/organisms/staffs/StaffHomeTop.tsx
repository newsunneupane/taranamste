...
// ✨ Accept pendingUsers as a prop
const StaffHomeTop = ({ pendingUsers = [] }: { pendingUsers?: any[] }) => {
    ...
    return (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-card p-4 md:p-5 rounded-2xl shadow-sm border border-border transition-colors duration-500">
            ...
            <Button
                onClick={() => openStaffForm({ pendingUsers })}
                className="w-full sm:w-auto bg-primary text-text-invert hover:opacity-90 shadow-glow font-bold py-2 px-5 rounded-xl text-xs transition-all"
            >
                + Add Employee
            </Button>
        </div>
